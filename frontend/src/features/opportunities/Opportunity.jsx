...
// 2fr_1.2fr_1.2fr_0.4fr
function Opportunity({ opportunity }) {
  const navigate = useNavigate();
  return (
    <div
      onClick={() => navigate(`${opportunity.id}`)}
      className="grid grid-cols-[1.6fr_1fr_0.6fr_1fr_0.4fr] gap-[1.2rem] items-center px-[2rem] py-[1.2rem] border-b-[.1rem] cursor-pointer hover:bg-slate-50"
      key={opportunity.id}
    >
      <div>
      ...
      <p className="text-[1.4rem]">{opportunity.opportunity_type}</p>
