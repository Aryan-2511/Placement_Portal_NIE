...
func GetPlacedStudents(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	query := `SELECT id,usn,opportunity_id,name,email,branch,batch,company,package,placement_date,contact,placement_type FROM placed_students`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&student.ID, &student.USN, &student.OpportunityID, &student.Name, &student.Email, &student.Branch, &student.Batch, &student.Company, &student.Package, &student.PlacementDate, &student.Contact, &student.PlacementType); err != nil {
